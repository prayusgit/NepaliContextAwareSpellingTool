...
def highlight_corrections(original_sentence, corrected_sentence, corrections):
    highlighted_sentence = []
    for word in corrected_sentence.split():
        if word in corrections.values():  # Check if the word was corrected
            highlighted_sentence.append(f"<span style='background-color: #C2FFC7;'>{word}</span>")
        else:
...
def main():
    ...
    st.write("Correct spelling mistakes in Nepali sentences and see the probabilities for candidates.")
    ...
    if st.button("Check Spelling"):
        if sentence.strip():
            ...
            highlighted = highlight_corrections(sentence, corrected_sentence, corrections)
